...
from protsupport.modules.structured_energy import StructuredEnergy, StupidEnergy, DistanceStupidEnergy, TertiaryStupidEnergy, DilatedStupidEnergy
...
class EBMNet(ProteinNetKNN):
  ...
  def __getitem__(self, index):
    ...
    distances = distances.permute(1, 0).contiguous()
...
#    return result
  ...
  def each_generate(self, data, gt):
    with torch.no_grad():
      ters = data.cpu()
      dist = (ters[:, :, None, :] - ters[:, :, :, None]).norm(dim=1, keepdim=True)
...
if __name__ == "__main__":
  ...
  net = SDP(
    TertiaryStupidEnergy(depth=8, down=2, shape=64)
  )
  ...
  training = EBMTraining(
    ...
    device="cuda:0",
    network_name="distance-ebm/stoopid-ebm-tertiary-46",
    verbose=True
  ).load()
