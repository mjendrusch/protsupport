...
class TotalLoss(nn.Module):
  ...
  def forward(self, inputs, targets):
    ...
    msum = mask.float().sum()
    m2sum = mask2d.float().sum()
    loss = (self.ce(inputs[0], angle) * mask.float()).sum() / msum
    loss += (self.ce(inputs[1], dihedral) * mask.float()).sum() / msum
    print(self.kl(inputs[2], pssm).shape)
    loss += (self.kl(inputs[2], pssm) * mask[:, None].float()).sum(dim=1).sum() / msum
    ...
    loss += (self.kl(inputs[9], pssm) * mask[:, None].float()).sum(dim=1).sum() / msum
    loss = (self.ce(inputs[10], angle) * mask.float()).sum() / msum
    loss += (self.ce(inputs[11], dihedral) * mask.float()).sum() / msum
    loss += (self.kl(inputs[12], pssm) * mask[:, None].float()).sum(dim=1).sum() / msum
    return loss
...
if __name__ == "__main__":
  ...
  net = DistancePredictor(pair_depth=10, seq_depth=5)
