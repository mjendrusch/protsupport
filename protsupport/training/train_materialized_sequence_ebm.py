...
class EBMTraining(EnergyTraining):
  def prepare(self):
    ...
    (distance, ground_truth), *_ = self.data[index]
...
class MaterializedEnergy(nn.Module):
  ...
  def forward(self, data):
    if len(data) == 2:
      ...
      inputs, sequence, scale = data
      sequence = (sequence - sequence.mean(dim=(1, 2), keepdim=True)) / sequence.std(dim=(1, 2), keepdim=True)
      sequence = sequence * scale[:, 0][:, None, None] + scale[:, 1][:, None, None]
      tmp = torch.zeros_like(sequence)
      ind_b = torch.arange(tmp.size(0), dtype=torch.long, device=tmp.device)
      ind_s = torch.arange(tmp.size(2), dtype=torch.long, device=tmp.device)
      amx = sequence.argmax(dim=1)
    ...
    # inputs = torch.cat((
    #   inputs,
    #   sequence[:, :, None, :].expand(*sequence.shape, sequence.size(2)),
    #   sequence[:, :, :, None].expand(*sequence.shape, sequence.size(2)),
    # ), dim=1)
